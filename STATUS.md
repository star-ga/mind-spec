...
> **Last Updated:** 2026-05-19 · spec v1.0 · tracking compiler **0.6.4 — RFC 0006 mind-blas Track B increment 2** on top of the v0.6.1 bootstrap fixed-point, v0.6.2 negative-literal fix, and v0.6.3 (Track B inc 1, native MLIR vector-dialect `dot_f32`). v0.6.4 adds the `std-surface`-gated Q16.16 vector path (`dot_q16_v`: `vector<8xi64>` widen-mul-`shrsi 16`-accumulate + associative `vector.reduction <add>`), `Instr::VecStore`, and f32 `dot_l1_f32_v`/`dot_linf_f32_v`. **The cross-arch bit-identity gate #57 is CLOSED for the thesis-pure vector dot**: `dot_q16_v` is byte-identical to the Track A scalar oracle at every length {0,1,2,7,8,9,15,16,17,31,32,33,1024,4096,65537} (single-x86-host scope, as Track A §5.2; full Linux-x86↔macOS-ARM↔CUDA hardware contract is the remaining #57 portion). Bootstrap fixed-point **byte-identical** (10,889 bytes / 206 SSA — bootstrap uses no vector ops); bench-gate 0.0% (default-feature binary byte-identical sha256 c130db3↔8e4b925, all Track B code feature-gated); 519 std-surface tests; f32 within 1e-4 rel of f64 oracle to 1M. Cumulative pure-MIND-discipline compiler bugs surfaced + closed: **11**. The Rust implementation is decorative — required only for the initial bootstrap. **Mindcraft sequencing gate CLEARED 2026-05-19** (`mind@a29726c`): the compiler-repo RFC 0007 pure-MIND `mindc fmt`/`lint`/`check` toolchain is unblocked — native-encoder end-to-end measurement + numerical correctness (cosine 0.999996 / top-5 0.9975, Q16.16 bit-identity preserved) and the criterion benchmark publication are both complete; build may begin. Documentation/sequencing milestone only — compiler tracking unchanged at v0.6.4 (no new tag).
