...
> **Last Updated:** 2026-05-28 · spec v1.0 · tracking compiler **v0.7.0 + unreleased -- credibility-ladder rung 3 graduation: Mindcraft RFC 0007 fully shipped; RFC 0008 all 7/7 phases (KEYSTONE: cargo retired from pure-MIND compile loop); RFC 0010 Phases A/B/C + J-A/J-B shipped (extern "C" + SysV + Win64 ABI; region { } region-interior + GenRef region-exterior memory tiers), G1 dropped vestigial deps; RFC 0011 Phase A (std.async Scheduler-injection + ReplayScheduler); RFC 0012 tensor-native syntax Phases A (shape-typed Tensor<dtype,[dims]>) + B (`@`/`.*`/`.T`/reductions → std.blas) shipped, Phase C annotation checks shipped on the single canonical `#[name]` attribute form (C.0 threading + C.1 enforced `determinism::unknown_target`/`float_in_q16_fn`/`nondeterministic_in_deterministic` + C.2 implicit dtype-suffix determinism + `module { }` descent); 13 stdlib modules; mindc doc; standalone binary release.** See [`star-ga/mind CHANGELOG`](https://github.com/star-ga/mind/blob/main/CHANGELOG.md) for the full per-version record (the post-0.7.0 RFC 0010 J / 0011 A / 0012 A-B-C work is in the Unreleased section).
...
## Toolchain Features (mindc v0.7.0)
...
| Evidence `trace_hash` anchored on canonical `mic@1` IR | RFC 0016 GAP-1 | ✅ Shipped | v0.7.x (post-tag, mind@`db5cb76`) | `mind/src/ir/evidence.rs` |
...
| `mindc verify` CLI + `mic@2.x` → `mind-model@2` demotion + CI oracle gate | RFC 0021 steps 4–6 | 🚧 In progress | targets v0.7.1+ | [`docs/rfcs/0021-canonical-ir-unification.md`](https://github.com/star-ga/mind/blob/main/docs/rfcs/0021-canonical-ir-unification.md) |
