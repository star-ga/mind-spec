...
> **Last Updated:** 2026-06-14 · spec v1.0 · tracking compiler **v0.8.1 (released) — the pure-MIND self-host front-end is byte-exact on the full 66/66 gap corpus, the `mic@3` binary-IR self-host fixed-point is byte-identical, and RFC 0012 tensor-native surface syntax is shipped.** v0.8.0/v0.8.1 advance the self-host fixed-point from the `mic@1` MLIR-text layer to the **canonical `mic@3` binary `IRModule`** — the layer the evidence chain's `trace_hash` anchors on — and prove byte-exact front-end compilation across the 66-fixture gap corpus (a fuzz sweep of arbitrary programs, not just `main.mind`), with **0 wrong-bytes** (the cardinal never-a-silent-miscompile invariant) and **0 fail-closed**; CI-gated by `mic3_flip_smoke.py` and `gap_corpus_smoke.py` (FLOOR ratcheted to 66). Release gate `#306` cleared (the `std.string`/`std.sha256`/`std.toml` byte-precise-store migration + keystone re-bless). The earlier v0.7.1 surface still holds: the high-level execution surface EXECUTES on the shipped compiler with `std-surface` the shipped default — ten desugaring "bricks", each keystone-verified 7/7 across substrates (integer/literal match, enum-discriminant match, `Option`/payload match, string literals, struct field-write, nested-struct + borrow field access, method-as-field accessors `s.len()`, and method-with-args UFCS `v.push(x)` → `vec_push(v, x)`, with unresolved methods failing loud); cross-substrate Q16.16 byte-identity preserved across the flip; unpromoted constructs (by-value tuple/aggregate returns, `region { }` interior) re-gated behind `std-surface-experimental` and **fail loud by default**; generics have no syntax yet (parser-rejected); `--no-default-features` restores the low-level-only subset. Prior v0.7.0 work still holds: Mindcraft RFC 0007 fully shipped; RFC 0008 all 7/7 phases (KEYSTONE: cargo retired from pure-MIND compile loop); RFC 0010 Phases A/B/C + J-A/J-B shipped (extern "C" + SysV + Win64 ABI; region { } region-interior + GenRef region-exterior memory tiers), G1 dropped vestigial deps; RFC 0011 Phase A (std.async Scheduler-injection + ReplayScheduler); RFC 0012 tensor-native syntax Phases A (shape-typed Tensor<dtype,[dims]>) + B (`@`/`.*`/`.T`/reductions → std.blas) shipped, Phase C annotation checks shipped on the single canonical `#[name]` attribute form (C.0 threading + C.1 enforced `determinism::unknown_target`/`float_in_q16_fn`/`nondeterministic_in_deterministic` + C.2 implicit dtype-suffix determinism + `module { }` descent); 13 stdlib modules; mindc doc; standalone binary release. See [`star-ga/mind CHANGELOG`](https://github.com/star-ga/mind/blob/main/CHANGELOG.md) for the full per-version record.
...
## Toolchain Features (mindc v0.8.1)
...
## Toolchain Features (mindc v0.8.1 compiler reference)
