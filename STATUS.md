...
> **Last Updated:** 2026-05-19 · spec v1.0 · tracking compiler **0.6.2 — negative-literal correctness fix on top of the Bootstrap fixed-point**. The pure-MIND mindc `libmindc_mind.so` compiles its own 1,084-LOC `.mind` source byte-identically (10,889 bytes of MLIR text, 206 SSA values) to mindc-Rust — v0.6.1 closed that loop; v0.6.2 fixes compiler bug #11 (bare negative integer/float literals + any unary-minus expression lowered to `const 0`), surfaced by the pure-MIND lookup-table work and fixed without disturbing the fixed-point (oracle byte-identical; bootstrap sources contain zero negative literals) and with the bench-gate *improved* (−0.2 % to −1.8 %). The Rust implementation is decorative — required only for the initial bootstrap.
